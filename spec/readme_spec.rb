...
describe "README" do
  ...
  it "verifies number formatting" do
    nbsp = "\xC2\xA0"
    1337.localize(:es).to_s.should match_normalized("1 337")
    spaces(1337.localize(:es).to_currency.to_s).should match_normalized("1 337,00 $")
    spaces(1337.localize(:es).to_currency.to_s(:currency => "EUR").to_s).should match_normalized("1 337,00 €")
    1337.localize(:es).to_percent.to_s.should match_normalized("1 337%")
    1337.localize(:es).to_percent.to_s(:precision => 2).should match_normalized("1 337,00%")
    1337.localize(:es).to_decimal.to_s(:precision => 3).should match_normalized("1 337,000")

    num = TwitterCldr::Localized::LocalizedNumber.new(1337, :es)
    spaces(num.to_currency.to_s).should match_normalized("1 337,00 $")
  end
