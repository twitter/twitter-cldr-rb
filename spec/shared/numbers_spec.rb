...
describe TwitterCldr::Shared::Numbers do
  describe "#symbols" do
    ...
    it 'returns numerical symbols for default locale' do
      ...
      allow(TwitterCldr).to receive(:get_locale_resource).with(:ja, :numbers).and_return(ja: { numbers: { symbols: } })
      expect(described_class.symbols).to eq(symbols)
    end

    it 'returns numerical symbols for default locale' do
      allow(TwitterCldr).to receive(:get_locale_resource).with(:np, :numbers).and_return(np: { numbers: { symbols: } })
      expect(described_class.symbols(:np)).to eq(symbols)
    end

    it 'converts locale' do
      allow(TwitterCldr).to receive(:get_locale_resource).with(:'zh-Hant', :numbers).and_return('zh-Hant': { numbers: { symbols: } })
