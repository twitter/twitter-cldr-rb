...
TwitterCldr.NumberFormatter = class NumberFormatter
  ...
  parse_number: (number, options = {}) ->
    precision = options.precision || this.precision_from(number)
  ...
  class BaseHelper
    interpolate: (string, value, orientation = "right") ->
      ...
      (string[0...start] + value + string[(length + 1)..-1]).replace("#", "")

  class IntegerHelper extends BaseHelper
    ...
    apply: (number, options = {}) ->
      this.format_groups(this.interpolate(@format, number.toFixed(0)))
    ...
    format_groups: (string) ->
      ...
      string = string[0...(string.length - cur_token.length)]

      while string.length > @groups.last
