...
module TwitterCldr
  module Formatters
    module RuleBasedNumberFormatter
      @formatters[:ms] = Malay = Class.new do
        class << self
          ...
          def renderSpelloutCardinal(n)
            ...
            if (n >= 1000000) then
              return ((renderSpelloutCardinal((n / 1000000.0).floor) + " juta") + ((n == 1000000) ? ("") : ((" " + renderSpelloutCardinal((n % 100000))))))
            end
            if (n >= 2000) then
          ...
          def renderSpelloutOrdinal(n)
            ...
            return n.to_s if is_fractional and (n > 1)
            return ("ke" + renderSpelloutCardinal(n)) if (n >= 2)
