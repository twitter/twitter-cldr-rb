...
TwitterCldr.Bidi = (function() {
  ...
  Bidi.prototype.reorder_visually = function() {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (!this.is_even(level)) {
        lowest_odd = this.min([lowest_odd, level]);
      }
    }
  ...
  Bidi.prototype.compute_explicit_levels = function() {
    ...
    for (i = _j = 0; 0 <= size ? _j <= size : _j >= size; i = 0 <= size ? ++_j : --_j) {
      ...
      len = next_fmt - input;
      this.arraycopy(this.levels, input, this.levels, output, len);
      this.arraycopy(this.types, input, this.types, output, len);
      output += len;
      input = next_fmt + 1;
    }
  ...
  Bidi.prototype.resolve_weak_types = function() {
    ...
    for (run_idx = _i = 0; 0 <= run_count ? _i < run_count : _i > run_count; run_idx = 0 <= run_count ? ++_i : --_i) {
      ...
      sor = this.is_even(this.max([previous_level, level])) ? "L" : "R";
      next_level = run_idx === (run_count - 1) ? this.base_embedding : this.get_run_level(run_idx + 1) || 0;
      eor = this.is_even(this.max([level, next_level])) ? "L" : "R";
      prev_type = sor;
  ...
  Bidi.prototype.resolve_neutral_types = function() {
    ...
    for (run = _i = 0; 0 <= run_count ? _i < run_count : _i > run_count; run = 0 <= run_count ? ++_i : --_i) {
      ...
      embedding_direction = this.is_even(level) ? "L" : "R";
      sor = this.is_even(this.max([previous_level, level])) ? "L" : "R";
    ...
    if ((this.formatter_indices != null) && this.formatter_indices.length > 0) {
      ...
      for (index = _i = _ref = this.formatter_indices.length - 1; _ref <= 0 ? _i <= 0 : _i >= 0; index = _ref <= 0 ? ++_i : --_i) {
        ...
        if (next_fmt + 1 < this.levels.length) {
          this.arraycopy(this.levels, input, this.levels, next_fmt + 1, len);
        }
        ...
        this.levels[output] = this.max([left_level, right_level]);
      }
    }
    return this.length = this.levels.length;
  };

  Bidi.prototype.arraycopy = function(orig, orig_index, dest, dest_index, length) {
  ...
  Bidi.prototype.max = function(arr) {
  ...
  Bidi.prototype.min = function(arr) {
  ...
  Bidi.prototype.is_even = function(num) {
    return num % 2 === 0;
  };

  Bidi.prototype.is_odd = function(num) {
    return num % 2 === 1;
  };
