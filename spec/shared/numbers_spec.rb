...
describe Numbers do
  describe "#symbols" do
    ...
    it 'returns numerical symbols for default locale' do
      ...
      stub(TwitterCldr).get_resource(:jp, :numbers) { { :jp => { :numbers => { :symbols => symbols } } } }
      Numbers.symbols.should == symbols
    end

    it 'returns numerical symbols for default locale' do
      stub(TwitterCldr).get_resource(:np, :numbers) { { :np => { :numbers => { :symbols => symbols } } } }
      Numbers.symbols(:np).should == symbols
    end

    it 'converts locale' do
      stub(TwitterCldr).get_resource(:'zh-Hant', :numbers) { { :'zh-Hant' => { :numbers => { :symbols => symbols } } } }
      Numbers.symbols('zh-tw').should == symbols
    end

    it 'returns nil if the resource is missing' do
      stub(TwitterCldr).get_resource(:nop, :numbers) { nil }
