...
module TwitterCldr
  module Collation
    ...
    class ImplicitCEGenerator

      SECONDARY_AND_TERTIARY = 5

      def get_collation_element(code_point)
        [get_implicit_primary(swapCJK(code_point) + 1), SECONDARY_AND_TERTIARY, SECONDARY_AND_TERTIARY]
      end
      ...
      #
      def get_implicit_primary(cp)
        last0 = cp - MIN_4_BOUNDARY

        if last0 < 0
          last1 = cp / FINAL_3_COUNT
          last0 = cp % FINAL_3_COUNT

          last2 = last1 / MEDIAL_COUNT
          last1 %= MEDIAL_COUNT

          # spread out, leaving gap at start
          last0 = MIN_TRAIL + last0 * FINAL_3_MULTIPLIER

          # offset
          last1 += MIN_TRAIL
          last2 += MIN_PRIMARY

          (last2 << 16) + (last1 << 8) + last0
        else
          last1 = last0 / FINAL_4_COUNT
          last0 %= FINAL_4_COUNT
      ...
      def swapCJK(cp)
        if cp >= CJK_BASE
          return cp - CJK_BASE                                      if cp < CJK_LIMIT
          return cp + NON_CJK_OFFSET                                if cp < CJK_COMPAT_USED_BASE
          return cp - CJK_COMPAT_USED_BASE + (CJK_LIMIT - CJK_BASE) if cp < CJK_COMPAT_USED_LIMIT
          return cp + NON_CJK_OFFSET                                if cp < CJK_B_BASE
          return cp                                                 if cp < CJK_B_LIMIT # non-BMP-CJK
          return cp + NON_CJK_OFFSET                                if cp < CJK_C_BASE
          return cp                                                 if cp < CJK_C_LIMIT # non-BMP-CJK
          return cp + NON_CJK_OFFSET                                if cp < CJK_D_BASE
          return cp                                                 if cp < CJK_D_LIMIT # non-BMP-CJK

          return cp + NON_CJK_OFFSET # non-CJK
        end

        return cp + NON_CJK_OFFSET if cp < CJK_A_BASE
        return cp - CJK_A_BASE + (CJK_LIMIT - CJK_BASE) + (CJK_COMPAT_USED_LIMIT - CJK_COMPAT_USED_BASE) if cp < CJK_A_LIMIT

        cp + NON_CJK_OFFSET # non-CJK
      end
